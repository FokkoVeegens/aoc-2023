...
namespace Day11
{
	public class Grid<T> where T : struct
	{
        ...
        // Insert a row at the specified index and fill it with the specified value
        public Grid<T> InsertRow(int index, T value)
        {
            var newGrid = new Grid<T>(Width, Height + 1);
            for (var x = 0; x < Width; x++)
            {
        ...
        // Insert a column at the specified index and fill it with the specified value
        public Grid<T> InsertColumn(int index, T value)
        {
            var newGrid = new Grid<T>(Width + 1, Height);
            for (var y = 0; y < Height; y++)
            {
        ...
        // Find all rows that only contain the specified value
        public IEnumerable<int> FindRows(T value)
        {
            for (var y = 0; y < Height; y++)
            {
                var row = GetRow(y);
                if (row.All(v => v.Equals(value)))
                {
                    yield return y;
                }
            }
        ...
        // Find all columns that only contain the specified value
        public IEnumerable<int> FindColumns(T value)
        {
            for (var x = 0; x < Width; x++)
            {
                var column = GetColumn(x);
                if (column.All(v => v.Equals(value)))
                {
                    yield return x;
                }
            }
